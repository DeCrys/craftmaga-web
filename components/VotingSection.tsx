...
// Ikony pro různé typy informací
...
const VotingSection = () => {
    ...
    () => [
      {
        id: "czech-craft",
        name: "Czech-Craft",
        url: "https://czech-craft.eu/server/crafmaga/vote/",
        reward: "1 coin + Klíč",
      ...
      },
      {
        id: "minecraft-list",
        name: "Minecraft-list",
        url: "https://www.minecraft-list.cz/server/crafmaga/vote",
        reward: "1 coin + Klíč",
  ...
  async function loadSiteLiveData(site: VotingSite) {
    try {
      ...
      let data: VotingApiResponse | null = null;

      if (site.type === "czech-craft" && site.slug) {
        const res = await fetch(`/api/czech-craft/${site.slug}`);
        data = (await res.json()) as VotingApiResponse;
      } else if (site.type === "craftlist" && site.token) {
        const res = await fetch(`/api/craftlist/${site.token}`);
        data = (await res.json()) as VotingApiResponse;
      } else if (site.type === "minecraftlist" && site.token) {
        const res = await fetch(`/api/minecraftlist/${site.token}`);
        data = (await res.json()) as VotingApiResponse;
      } else if (site.type === "serverlist" && site.slug) {
        const res = await fetch(`/api/minecraft-list/${site.slug}`);
        data = (await res.json()) as VotingApiResponse;
      }
