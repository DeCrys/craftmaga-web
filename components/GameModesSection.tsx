...
import { Sword, Crown, Shield, Gamepad2 } from 'lucide-react'
import { useEffect, useState, useMemo } from "react"

// Typ pro hráče z Dynmap API
...
const GameModesSection = () => {
  // Pole herních módů nyní memoizované, aby se neměnilo při každém renderu
  const initialGameModes = useMemo(() => [
    {
      icon: Crown,
      title: "Spawn",
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      ...
      for (const mode of initialGameModes) {
        ...
        try {
          const res = await fetch(`/api/dynmap/up/world/${mode.world}/`)
          ...
          const onlinePlayers =
            data.players?.filter((p) => !p.world || p.world === mode.world).length ??
            data.playersOnline ??
            0
